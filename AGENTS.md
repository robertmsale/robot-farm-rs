...
- `system::init_system_state()` runs at startup (after DB init) to seed the strategy state and queue coordinator. The server always boots in `Planning` mode until another strategy is persisted via the `/strategy` endpoints. Strategies influence orchestrator hints, but the work queue is controlled exclusively through the `/queue` API (and therefore the play/pause button) and always starts paused.
...
- Worker-task assignments live in the queue coordinator. Helpers ensure a worker only has one active task, keep track of idle workers, and compute orchestrator hints based on the current strategy focus. Middleware/process-manager hooks (COMPLETE_TASK interception, merge validation, post-turn checks) should call into this coordinator to add events and release assignments. Post-turn checks always run when a worker reports `COMPLETE_TASK` regardless of the active strategy.
...
- WebSocket subscribers receive an initial `workers_snapshot` followed by realtime events: `feed_entry` (new feed rows), `queue_state` (pause/resume), and `strategy_state` (active strategy id plus focus). The Flutter app uses these to keep multiple devices in sync without manual refreshes.
...
- The play/pause button only drives `/queue`; strategies never start or stop the queue automatically.
- Feed action menus use Material popup menus (⋮) and expose “Run command”, “Enqueue message”, and “Modify queue”. The worker pane header mirrors the orchestrator pane and its actions.
- The queue management sheet displays live data from `/message_queue`, supports reorder (`PATCH /message_queue/{id}/insert`), deletion, and clearing the queue. Manual enqueue sheets call `POST /message_queue` so user messages immediately hit the feed and the queue.
