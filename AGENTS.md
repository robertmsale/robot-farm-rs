...
- `server/`: Axum-based API server. Routes live in `server/src/routes`, and each domain (tasks, task groups, workers, message queue, feed, config, git) has its own module. Shared functionality (e.g., git helpers) is placed in `server/src/shared`.
...
- Startup calls `shared::git::ensure_non_bare_repo` to panic early if `<workspace>/staging` is missing or bare.
