...
- **Execute focused workflows.** In this sandboxed environment, you will be working on projects that require specific tooling which is not available on your system (e.g. `cargo`). You will be provided with tooling via MCP. Use `robot_farm.project_command_list({})` to retrieve project-specific tooling with information about what the command does. Then use `robot_farm.project_command_run({command_id: string})` to execute the command. Many of these commands are executed automatically when you send a `COMPLETE_TASKS` intent.

## Intent discipline

- **`COMPLETE_TASKS`** – once work is done, list the closed task slugs, highlight diffs/tests, and provide a short commit-style summary (≤72 characters) describing the changes so the server can auto-commit on your behalf.
